...
// 六个金智接口（均 POST，credentials:include 带 VPN 会话票 cookie）
...
async function runImportFlow() {
    ...
    try {
        ...
        // 1. 当前学期（自动获取，无需用户选择）
        const sem = await fetchCurrentSemester();
        if (!sem) {
            AndroidBridge.showToast("未获取到当前学期，请检查是否已登录并停留在课表页");
            return;
        }
